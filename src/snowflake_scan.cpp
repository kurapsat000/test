...
static unique_ptr<FunctionData> SnowflakeScanBind(ClientContext &context, 
    ...
    try {
        ...
        ArrowTableFunction::PopulateArrowTableType(context.db->config, arrow_table, bind_data->schema_wrapper, names, return_types);
...
static unique_ptr<GlobalTableFunctionState> SnowflakeScanInitGlobal(ClientContext &context, 
    ...
    try {
        ...
        AdbcStatusCode status = AdbcStatementNew(bind_data.connection->GetConnection(), 
                                               &global_state->statement, &error);
        if (status != ADBC_STATUS_OK) {
            throw IOException("Failed to create statement");
        }
...
static void SnowflakeScanExecute(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
    ...
    ArrowTableFunction::ArrowToDuckDB(local_state, bind_data.arrow_convert_data, output, 
                                     local_state.chunk_offset, true);
...
TableFunction GetSnowflakeScanFunction() {
    TableFunction snowflake_scan("snowflake_scan", {LogicalType::VARCHAR, LogicalType::VARCHAR}, 
                                SnowflakeScanExecute, SnowflakeScanBind, 
                                SnowflakeScanInitGlobal, SnowflakeScanInitLocal);
