...
void SnowflakeStoreCredentialsFunction(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    try {
        ...
        std::string result_msg = "Credentials stored successfully for profile: " + profile +
                                 " (username: " + user + ", account: " + acc + ", warehouse: " + wh +
...
ScalarFunction GetSnowflakeStoreCredentialsFunction() {
    return ScalarFunction(
        "snowflake_store_credentials",
        {LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR, 
         LogicalType::VARCHAR, LogicalType::VARCHAR, LogicalType::VARCHAR},
        LogicalType::VARCHAR,
        SnowflakeStoreCredentialsFunction
    );
}

ScalarFunction GetSnowflakeListProfilesFunction() {
    return ScalarFunction(
        "snowflake_list_profiles",
        {},
        LogicalType::VARCHAR,
        SnowflakeListProfilesFunction
    );
}

ScalarFunction GetSnowflakeValidateCredentialsFunction() {
    return ScalarFunction(
        "snowflake_validate_credentials",
        {LogicalType::VARCHAR},
        LogicalType::BOOLEAN,
